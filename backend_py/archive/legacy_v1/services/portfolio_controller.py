...
def solve_forecast_portfolio(
    ...
    Args:
        ...
        time_budget: Total time budget in seconds (OR ConfigV4 object for back-compat)
        seed: Random seed for determinism
        config: Optional ConfigV4 object (preferred way to pass config)
    
    Returns:
        SolveResultV4 with solution and KPIs
    """
    # Backward compatibility: if time_budget is actually a ConfigV4 object, use it
    if hasattr(time_budget, 'time_limit_phase1'):  # Duck typing for ConfigV4
        config = time_budget
        # Use config's time limits if reasonable, else default
        time_budget = config.anytime_budget if hasattr(config, 'anytime_budget') else 30.0

    result = run_portfolio(tours, time_budget, seed, config=config)
