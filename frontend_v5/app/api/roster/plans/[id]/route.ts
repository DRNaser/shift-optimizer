/**
 * BFF Route: Roster Plan Detail API
 *
 * Proxies requests to /api/v1/roster/plans/{id} on the backend
 */
...
export async function GET(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    const response = await fetch(`${BACKEND_URL}/api/v1/roster/plans/${id}`, {
      headers: {
